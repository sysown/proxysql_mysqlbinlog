...
void daemonize_phase1(char *argv0, std::string pid_file) {
	int rc;
...
void usage(const char* name) {
	std::cout << "Usage: " << name << " -h <mysql host> -u <mysql user> -p <mysql password> -P <mysql port> -l <listen port> -L <log file> -i <Pid file>" << std::endl;
}
...
int main(int argc, char** argv) {
	...
	int c;
	while (-1 != (c = ::getopt(argc, argv, "fh:u:p:P:l:L:i:"))) {
		switch (c) {
	...
	if (foreground==false) {
	daemonize_phase1((char *)argv[0], pid_file);
